...
public class KvpPrepPepActionHelper implements BaseKvpVisitAction.KvpVisitActionHelper {

    private String enrolled_to_prep;
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            enrolled_to_prep = CoreJsonFormUtils.getValue(jsonObject, "enrolled_to_prep");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(enrolled_to_prep))
