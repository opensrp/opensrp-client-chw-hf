...
public class KvpOtherServicesActionHelper implements BaseKvpVisitAction.KvpVisitActionHelper {

    private String other_services_referrals_provided;
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            other_services_referrals_provided = CoreJsonFormUtils.getValue(jsonObject, "other_services_referrals_provided");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(other_services_referrals_provided))
