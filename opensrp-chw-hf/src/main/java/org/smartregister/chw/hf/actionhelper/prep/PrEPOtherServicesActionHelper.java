...
public class PrEPOtherServicesActionHelper implements BaseKvpVisitAction.KvpVisitActionHelper {

    private String health_edu_sti_provided;
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            health_edu_sti_provided = CoreJsonFormUtils.getValue(jsonObject, "health_edu_sti_provided");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(health_edu_sti_provided))
