...
public class KvpVmmcActionHelper implements BaseKvpVisitAction.KvpVisitActionHelper {

    private String vmcc_provided;
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            vmcc_provided = CoreJsonFormUtils.getValue(jsonObject, "vmcc_provided");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(vmcc_provided))
