...
public class KvpHealthEducationHelper implements BaseKvpVisitAction.KvpVisitActionHelper {

    private String health_education_provided;
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            health_education_provided = CoreJsonFormUtils.getValue(jsonObject, "health_education_provided");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(health_education_provided))
