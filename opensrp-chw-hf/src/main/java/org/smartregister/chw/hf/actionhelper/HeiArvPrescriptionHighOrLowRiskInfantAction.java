...
public class HeiArvPrescriptionHighOrLowRiskInfantAction implements BasePmtctHomeVisitAction.PmtctHomeVisitActionHelper {
    ...
    public HeiArvPrescriptionHighOrLowRiskInfantAction(MemberObject memberObject) {
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            provided_nvp_syrup = CoreJsonFormUtils.getValue(jsonObject, "provided_nvp_syrup");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public String evaluateSubTitle() {
        if (StringUtils.isBlank(provided_nvp_syrup))
            return null;

        return provided_nvp_syrup.equalsIgnoreCase("yes") ? context.getString(R.string.hei_arv_prescription_given) : context.getString(R.string.hei_arv_prescription_not_given);
    }
