...
public class KvpStiScreeningActionHelper implements BaseKvpVisitAction.KvpVisitActionHelper {
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            sti_screening = CoreJsonFormUtils.getValue(jsonObject, "sti_screening");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(sti_screening))
