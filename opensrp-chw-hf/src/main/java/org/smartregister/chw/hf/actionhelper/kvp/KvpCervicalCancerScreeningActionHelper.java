...
public class KvpCervicalCancerScreeningActionHelper implements BaseKvpVisitAction.KvpVisitActionHelper {

    private String cervical_cancer_screening;
    ...
    public void onPayloadReceived(String jsonPayload) {
        try {
            JSONObject jsonObject = new JSONObject(jsonPayload);
            cervical_cancer_screening = CoreJsonFormUtils.getValue(jsonObject, "cervical_cancer_screening");
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public BaseKvpVisitAction.Status evaluateStatusOnPayload() {
        if (StringUtils.isBlank(cervical_cancer_screening))
